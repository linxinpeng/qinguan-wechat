	...
	<view class="release">
		...
		<form>
			<form-item label="服务类型" required>
				<select-input v-model="form.serveType" :options="['找合作', '发需求']"></select-input>
			</form-item>
			...
			<form-item label="相关照片" desc="（最多3张，图中不能带联系方式,不超过200k)">
				<view class="upload">
					<g-upload ref='gUpload' :mode="form.imgs" @chooseFile='chooseFile' @imgDelete='imgDelete' :columnNum="3"></g-upload>
				</view>
			</form-item>
			...
			<button class="submit" type="default" @click="handleSend">确认发布</button>
		</form>
